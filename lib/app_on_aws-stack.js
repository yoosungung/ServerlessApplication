...
class AppOnAwsStack extends cdk.Stack {
  ...
  constructor(scope, id, props) {
      ...
      cors: [
        {
          ...
          allowedOrigins: [`http://${appUrl}`, 'http://localhost:8080'],
          maxAge: 600
        }        
      ]
    });
    ...
    const GetS3FileUrlFunction = new lambda.Function(this, 'GetS3FileUrlFunction', {
      code: lambda.Code.fromAsset(path.join(__dirname, '../src/server')),
      runtime: lambda.Runtime.NODEJS_14_X,
      memorySize: 128,
      timeout: cdk.Duration.seconds(10),
      handler: 's3files.GetS3FileHandler',
      environment: {
        USER_BUCKET: userBucket.bucketName
      },
      role: funcRole
    });
    const GetS3FileUrlIntegration = new integrations.LambdaProxyIntegration({ handler: GetS3FileUrlFunction });
    ...
    httpApi.addRoutes({
      ...
      integration: GetS3FileUrlIntegration
    });
    httpApi.addRoutes({
      path: '/api/file/{key+}',
      methods: [
